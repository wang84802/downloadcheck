...
def check_guid_loop(guid_list, raw_url, headers_dict, body_dict, method):
    ...
    for guid in guid_list:
        ...
        if method.upper() == "GET":
            ...
            print(f"❌ 不支援的 HTTP 方法: {method}，GUID: {guid}")
            continue

        if response.status_code != 200:
            arr_other_error_list.append(f"❌ API 回傳非 200 狀態碼: {response.status_code}，GUID: {guid}") # 無法檢查大小
            print(f"❌ API 回傳非 200 狀態碼: {response.status_code}，GUID: {guid}")
            continue
        ...
        if missing:
            ...
            print(f"❌ API 回傳資料缺少必要欄位: {missing}, {guid}")
            # 跳過後面檢查，直接進入下一個 GUID
            continue
        ...
        print(f"ZIP 檔案 URL: {zip_url}")
        print(f"預期檔案大小：{expected_size_bytes} Bytes")
        print(f"預期 MD5 簡碼：{expected_checksum}")
        ...
        if remote_size_bytes:
            ...
            print(f"實際檔案大小：{remote_size_bytes} Bytes ({remote_size_mb:.2f} MB)")
        else:
        ...
        if guid == 'pattern_Pignose_190103_3D_ChineseNewYear':
            ...
            if calculated_md5 == expected_checksum:
                ...
                print(f"❌ 檔案完整性驗證失敗，MD5 不符合 : {guid}\nzip_url : {zip_url} \
